...
func (s *userServiceImpl) ForgotPassword(email string) error {
	...
	if err := s.tokenRepo.DeleteTokensByUserAndType(user.UserID, model.TokenTypePasswordReset); err != nil {
		fmt.Printf("Warning: failed to delete old password reset tokens: %v\n", err)
	}
...
func (s *userServiceImpl) ResetPassword(code string, newPassword string) error {
	if code == "" {
...
func (s *userServiceImpl) ChangePassword(userID string, oldPassword string, newPassword string) error {
	if userID == "" {
