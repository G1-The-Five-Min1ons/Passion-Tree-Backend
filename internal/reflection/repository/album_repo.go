...
func (r *repositoryImpl) GetAlbumByID(ctx context.Context, albumID string) (*model.Album, error) {
	query := `
		SELECT CONVERT(VARCHAR(36), album_id) as album_id, COALESCE(album_name, '') as album_name, tree_count, COALESCE(cover_image_url, '') as cover_image_url, COALESCE(create_at, GETDATE()) as create_at, COALESCE(last_edit, GETDATE()) as last_edit, CONVERT(VARCHAR(36), user_id) as user_id
		FROM tree_album
		WHERE album_id = @p1
	`
