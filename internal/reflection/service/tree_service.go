...
func (s *serviceImpl) CreateTree(ctx context.Context, req model.CreateTreeRequest) (*model.TreeResponse, error) {
	...
	return &model.TreeResponse{
		...
		Status:        model.ComputeTreeStatus(tree.Difficulties, tree.LastReflectAt, tree.IsPause, tree.PausedAt),
...
func (s *serviceImpl) GetTreeByID(ctx context.Context, treeID string) (*model.Tree, error) {
	...
	tree.Status = model.ComputeTreeStatus(tree.Difficulties, tree.LastReflectAt, tree.IsPause, tree.PausedAt)
...
func (s *serviceImpl) GetTreesByAlbumID(ctx context.Context, albumID string, includeNodes bool) (interface{}, error) {
	...
	if includeNodes {
		...
		for i := range treesWithNodes {
			treesWithNodes[i].Status = model.ComputeTreeStatus(
	...
	// Compute live status for each tree.
	for i := range trees {
		trees[i].Status = model.ComputeTreeStatus(
